...
export class MockUseKeyedState {
   ...
   */
  mockVal(mockKey: string, val: unknown) {
   ...
   */
  mockVals(mapping: Record<string, unknown>) {
  ...
  expectInitializedWith(key: string, value: unknown) {
    expect(this.initValues[key]).toEqual(value);
  }

  expectSetStateCalledWith(key: string, value: unknown) {
    expect(this.setState[key]).toHaveBeenCalledWith(value);
  }
}

export const mockUseKeyedState = (stateKeys: Record<string, string>) => new MockUseKeyedState(stateKeys);
